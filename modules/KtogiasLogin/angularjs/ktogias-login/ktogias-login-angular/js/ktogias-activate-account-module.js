...
angular.module('ktogiasactivateaccount', ['alerts'])
.run(['$window', '$templateCache', function($window, $templateCache){
    var serverVars = $window.serverVars['KtogiasLogin\\Controller\\KtogiasLoginAngularController\\ktogias-activate-account-module'];
...
.factory('ktogiasactivateaccountService', ['$window', '$http', '$rootScope', function($window, $http, $rootScope){
        var serverVars = $window.serverVars['KtogiasLogin\\Controller\\KtogiasLoginAngularController\\ktogias-activate-account-module'];
        function activateAccount(password, success, failure){
            $http.post('/login/ktogias-activate-account-json/activate', {
                password: password, 
...
.controller('ktogiasactivateaccountController', ['$scope', '$window', '$location','ktogiasactivateaccountService', 'alertsService', function($scope, $window, $location, ktogiasactivateaccountService, alertsService){
    var serverVars = $window.serverVars['KtogiasLogin\\Controller\\KtogiasLoginAngularController\\ktogias-activate-account-module'];
    var controller = this;
    var watchers = [];
    ...
    controller.errorMessages = {
        ...
        ACTIVE: 'Ο λογαριασμός είναι ήδη εργοποιημένος.',
    ...
    controller.clear = function(){
        ...
        $scope.activateForm.$setUntouched();
    };
    
    controller.activateAccount = function(){
        ktogiasactivateaccountService.activateAccount(controller.password, function(data){
    ...
    controller.checkPaswordStrength = function(){
        ...
        var warnings = {
            ...
            'A word by itself is easy to guess': 'Είναι εύκολο να μαντέψει κάποιος μια μόνο λέξη.',
        ...
        if (result.score >= serverVars.config['min-allowed-password-score']){
            $scope.activateForm.password.$setValidity('passwordStrength', true);
        }
        else {
            $scope.activateForm.password.$setValidity('passwordStrength', false);
